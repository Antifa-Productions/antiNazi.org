...
self.onactivate = function (event) {
    // remove caches beginning with variable strings such as "^icon" or "[sw.js]"
    // that aren't in expectedCaches
    event.waitUntil(caches.keys().then(function (cacheNames) {
        return Promise.all(cacheNames.map(function (cacheName) {
            if (![apple-touch-^]/[touch-icon-^]/[favicon^]/[index^]/[^js]/[manifest^]/.test(cacheName)) {
                return;
            }
...
self.onfetch = function (event) {
 ...
 event.respondWith(antinaziCache(event.request));} else if (/\antinazi\.org$/.test(requestURL.hostname)) {
...
if (request.headers.get('Accept') == 'x-cache/only') {
    ...
    return fetch(request.clone()).then(function (response) {
        return caches
          ...
          assetCache.keys().then(function(requests) {
...
function antinaziCacheResponse(request) {
  return caches.match(request).then(function(response) {
    ...
    return fetch(request.clone()).then(function(response) {
      caches.open('public').then(function(cache) {
